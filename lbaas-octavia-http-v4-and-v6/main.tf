...
resource "openstack_networking_secgroup_v2" "sg_icmp" {
  name        = "sg_icmp"
...
resource "openstack_networking_secgroup_rule_v2" "icmp" {
  ...
  security_group_id = openstack_networking_secgroup_v2.sg_icmp.id
}

resource "openstack_networking_secgroup_rule_v2" "icmp6" {
  direction         = "ingress"
  ethertype         = "IPv6"
  protocol          = "ipv6-icmp"
  remote_ip_prefix  = "::/0"
...
resource "openstack_networking_subnet_v2" "subnet_lbdemo_ipv6" {
  name            = "subnet_lbdemo_ipv6"
  network_id      = openstack_networking_network_v2.net_lbdemo.id
  subnetpool_id   = data.openstack_networking_subnetpool_v2.subnetpool_v6.id
  ip_version      = 6
}
...
resource "openstack_networking_router_interface_v2" "routerint_lbdemo_ipv6" {
  router_id = openstack_networking_router_v2.router_lbdemo.id
  subnet_id = openstack_networking_subnet_v2.subnet_lbdemo_ipv6.id
}
...
resource "openstack_compute_instance_v2" "instance_lbdemo" {
  ...
  security_groups = [
    ...
    openstack_networking_secgroup_v2.sg_icmp.name,
  ]
...
# IPv6 Loadbalancer
resource "openstack_lb_loadbalancer_v2" "lb_app_ipv6" {
  vip_subnet_id = openstack_networking_subnet_v2.subnet_lbdemo_ipv6.id
  name          = "application loadbalancer ipv6"
}

resource "openstack_lb_listener_v2" "lb_app_ipv6_listener" {
  protocol        = "HTTP"
  protocol_port   = 80
  loadbalancer_id = openstack_lb_loadbalancer_v2.lb_app_ipv6.id
  allowed_cidrs   = ["::/0"]
  insert_headers = {
    X-Forwarded-For  = "true"
    X-Forwarded-Port = "true"
...
resource "openstack_lb_pool_v2" "lb_app_ipv6_pool" {
  protocol    = "HTTP"
  lb_method   = "ROUND_ROBIN"
  listener_id = openstack_lb_listener_v2.lb_app_ipv6_listener.id
}

resource "openstack_lb_member_v2" "lb_app_ipv6_pool_members" {
  count = length(openstack_compute_instance_v2.instance_lbdemo)
  address = element(
    openstack_compute_instance_v2.instance_lbdemo.*.access_ip_v4,
    count.index,
  ...
  protocol_port = 80
  pool_id       = openstack_lb_pool_v2.lb_app_ipv6_pool.id
  name = element(
    openstack_compute_instance_v2.instance_lbdemo.*.name,
    count.index,
  )
...
resource "openstack_lb_monitor_v2" "lb_app_ipv6_monitor" {
  pool_id        = openstack_lb_pool_v2.lb_app_ipv6_pool.id
  type           = "HTTP"
  delay          = 10
  timeout        = 5
...
output "loadbalancer_http_ipv6" {
  value = "http://[${openstack_lb_loadbalancer_v2.lb_app_ipv6.vip_address}]"
}
