...
data "openstack_networking_port_v2" "port_instance_red" {
  device_id  = openstack_compute_instance_v2.instance_red.id
  network_id = openstack_compute_instance_v2.instance_red.network[0].uuid
}
...
resource "openstack_networking_floatingip_associate_v2" "fipas_red" {
  ...
  port_id     = data.openstack_networking_port_v2.port_instance_red.id
}
